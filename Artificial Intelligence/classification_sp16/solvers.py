...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
	#print grad_tensors
	for i in range(min(len(grad_tensors),len(param_vars))):
		updated=param_vars[i] - self.learning_rate*(grad_tensors[i])
		updates.append((param_vars[i],updated))
	print updates
        return updates
	...
	for i in range(min(len(grad_tensors),len(param_vars))):
		...
		updated=param_vars[i] - self.learning_rate*(grad_tensors[i]) + self.momentum*vel_vars[i]
		updates.append((vel_vars[i], -self.learning_rate*(grad_tensors[i]) + self.momentum*vel_vars[i]))
		updates.append((param_vars[i],updated))
        return updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            train_loss=session.run(loss_tensor, feed_dict={placeholders[i]:train_data[i] for i in range(len(train_data))})
            session.run(update_ops,feed_dict={placeholders[i]:train_data[i] for i in range(len(train_data))})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[i]:val_data[i] for i in range(len(train_data))})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
	Train_data=MinibatchIndefinitelyGenerator(train_data,1,True)
	Val_data=MinibatchIndefinitelyGenerator(val_data,1,True)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            Train_Data=Train_data.next()
            Val_Data=Val_data.next()
            train_loss=session.run(loss_tensor, feed_dict={placeholders[i]:Train_Data[i] for i in range(len(Train_Data))})
            session.run(update_ops,feed_dict={placeholders[i]:Train_Data[i] for i in range(len(Train_Data))})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[i]:Val_Data[i] for i in range(len(Val_Data))})
            train_losses.append(train_loss)
            val_losses.append(val_loss)
            if callback is not None: callback(model)
            self.display_progress(iter_, train_losses, val_losses)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
	...
	Train_data=MinibatchIndefinitelyGenerator(train_data,self.batch_size,True)
	Val_data=MinibatchIndefinitelyGenerator(val_data,self.batch_size,True)
